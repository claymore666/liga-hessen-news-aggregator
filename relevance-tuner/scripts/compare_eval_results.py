...
def print_comparison(results: list[dict]):
    ...
    if not results:
        ...
        print("  Run run_llm_eval.py or run_classifier_eval.py first.")
        return

    # Determine eval set version from first result
    eval_ver = results[0].get("eval_set_version", "?")
    total_items = results[0].get("metrics", {}).get("total_items", "?")

    print("=" * 120)
    print(f"EVAL RESULTS (eval_set v{eval_ver}, {total_items} items)")
    print("=" * 120)
...
def main():
    ...
    parser.add_argument("--type", choices=["llm", "classifier"], default=None,
