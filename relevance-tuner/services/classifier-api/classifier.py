...
class ParaphraseEmbedder(BaseEmbedder):
    """
    Paraphrase-multilingual-mpnet embedder.
    ...
    def __init__(self, max_length: int = 2000):
        super().__init__(
            ...
            max_length=max_length,
            task_prefix="",  # No prefix for this model
        )
...
class DuplicateStore:
    ...
    Uses paraphrase-multilingual-mpnet-base-v2 which is optimized for
    ...
    def add_item(
        ...
        """Add or update an item in the duplicate store."""
        existing = self.collection.get(ids=[item_id])
        if existing["ids"]:
            return False
