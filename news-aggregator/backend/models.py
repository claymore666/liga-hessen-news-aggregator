...
class LLMModelConfig(Base):
    """Configured LLM models with priority-based fallback."""
...
class Digest(Base):
    """Daily newsletter digest generated by LLM."""

    __tablename__ = "digests"

    id: Mapped[int] = mapped_column(primary_key=True)
    date: Mapped[datetime] = mapped_column(DateTime, unique=True)
    status: Mapped[DigestStatus] = mapped_column(String(20), default=DigestStatus.PENDING)
    item_ids: Mapped[list[int]] = mapped_column(JSONB, default=list)
    content: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    html_body: Mapped[str | None] = mapped_column(Text, nullable=True)
    text_body: Mapped[str | None] = mapped_column(Text, nullable=True)
    llm_model: Mapped[str | None] = mapped_column(String(100), nullable=True)
    llm_prompt_version: Mapped[int | None] = mapped_column(Integer, nullable=True)
